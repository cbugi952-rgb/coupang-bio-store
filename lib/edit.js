...
// structured output 제약: 모든 object에 additionalProperties:false + 전체 required.
// → "안 바꿈"은 빈 문자열/-1로 표현한다(선택 필드를 못 쓰기 때문).
export const OPS_SCHEMA = obj({
  ...
  ops: {
    ...
    items: {
      anyOf: [
        obj({ op: { const: "set_field" }, field: { type: "string", enum: FIELDS }, value: str("accent는 #RRGGBB. showImages는 true/false. 지우려면 빈 문자열") }),
        obj({ op: { const: "set_social" }, platform: { type: "string", enum: SOCIALS }, url: str("전체 URL. 지우려면 빈 문자열") }),
        obj({ op: { const: "set_stickers" }, stickers: { type: "array", items: str() } }),
        obj({ op: { const: "add_pick" }, topic: str(), verdict: str(), buttons: { type: "array", items: obj({ label: str(), url: str() }) } }),
        obj({ op: { const: "update_pick" }, index: { type: "integer" }, topic: str("안 바꾸면 빈 문자열"), verdict: str("안 바꾸면 빈 문자열"), status: { type: "string", enum: ["", "latest", "live", "hidden"] } }),
        obj({ op: { const: "remove_pick" }, index: { type: "integer" } }),
        obj({ op: { const: "add_block" }, type: { type: "string", enum: ["text", "video", "link"] }, title: str(), body: str(), url: str(), label: str() }),
        obj({ op: { const: "remove_block" }, index: { type: "integer" } }),
      ],
    },
  },
}, ["reply", "ops"]);
...
// 자연어 → ops. ANTHROPIC_API_KEY 필요. 생성과 같은 모델 정책(Haiku 기본).
export async function planEdits(site, message) {
  ...
  if (res.stop_reason === "max_tokens") throw new Error("응답이 잘렸습니다.");
  if (res.stop_reason === "refusal") throw new Error("이 요청은 처리할 수 없어요.");
  const block = res.content.find((b) => b.type === "text");
  if (!block || !block.text) throw new Error("응답이 비어 있습니다.");
  const out = JSON.parse(block.text);
  return { reply: String(out.reply || ""), ops: Array.isArray(out.ops) ? out.ops : [] };
}
