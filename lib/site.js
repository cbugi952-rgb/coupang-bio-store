...
export const RESERVED_HANDLES = new Set(["index", "app", "admin", "login", "api", "assets", "images", "terms", "privacy", "docs", "reset", "verify", "onboard"]);
...
// Rotate the standalone, site-scoped key without touching the site document.
// This is used by legacy sites that do not have an account/session owner yet.
export async function rotateSiteKey(handle) {
  handle = sanitizeHandle(handle);
  if (!handle) return null;
  const existingSite = await store().get(siteKey(handle));
  if (!existingSite) return null;

  const pointer = "siteapikey:" + handle;
  const previous = await store().get(pointer);
  const key = await issueKey(handle);
  await store().set(pointer, key);
  if (previous && previous !== key) {
    try { await store().del("apikey:" + previous); } catch (e) {}
  }
  return key;
}
