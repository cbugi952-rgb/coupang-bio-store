...
export async function createUser(email, password, handle) {
  ...
  const user = { id, email: String(email).trim(), salt, hash, handle, verified: false, createdAt: Date.now() };
...
// ── 이메일 인증 토큰 (verify:{token} → { userId, at }, TTL 24시간) ──
const VERIFY_TTL = 60 * 60 * 24;
export async function createVerifyToken(userId) {
  const token = "v_" + randomBytes(24).toString("base64url");
  await store().set("verify:" + token, { userId, at: Date.now() });
  try { await store().expire("verify:" + token, VERIFY_TTL); } catch (e) { /* mock/미지원 무시 */ }
  return token;
}
// 유효하면 userId 반환, 아니면 null. store expire 미지원 대비 age도 직접 검사.
export async function consumeVerifyToken(token) {
  if (!token) return null;
  let rec = null;
  try { rec = await store().get("verify:" + token); } catch (e) { rec = null; }
  if (!rec || !rec.userId) return null;
  if (rec.at && Date.now() - rec.at > VERIFY_TTL * 1000) { try { await store().del("verify:" + token); } catch (e) {} return null; }
  return rec.userId;
}
export async function deleteVerifyToken(token) { if (token) { try { await store().del("verify:" + token); } catch (e) {} } }
