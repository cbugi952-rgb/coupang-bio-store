# Onshelf — 자동화-네이티브 링크 인 바이오 (서비스 스펙)
...
## 6. CLI (`onshelf`) — 구현됨 (`cli/`, 의존성 0)

REST API 얇은 래퍼. **MCP 툴 8종과 1:1 대응.** 키는 `onshelf login`으로 저장(`~/.config/onshelf/config.json`, POSIX면 600). env `API_BASE`·`API_KEY`·`SITE_HANDLE` 로도 덮어씀(CI·일회성).
```bash
onshelf login --key <sk_live_… 또는 관리자비번> --handle kkanajae   # echo <키> | onshelf login 도 가능
onshelf picks list [--json]
onshelf picks add --ep EP14 --topic 텀블러 --coupang <url> --toss <url> --category 주방 --latest
onshelf picks update <id> --toss <url>       # 값 없는 --toss = 그 필드 지우기
onshelf picks rm <id>
onshelf picks latest <id>                     # 대표(최신) 지정
onshelf picks reorder <id1> <id2> ...
onshelf profile get [--json]                  # 공개 읽기(키 불필요)
onshelf profile set --notice "이번 주 신상 3종" --instagram <url>
onshelf stats [--json]
onshelf whoami | url | logout
```
...
**구현됨**
- **자동화 코어 (웨지 3종 완성)**: 세밀 REST API(`/api/sites/{handle}/*` — 픽 CRUD·set-latest·reorder·profile·stats) + **MCP 서버**(`mcp/`, 툴 8개) + **CLI**(`cli/onshelf`, 의존성 0, MCP 툴 1:1) + 사이트/키 프로비저닝(`/api/provision`) → 웹 GUI·MCP·CLI 세 클라이언트가 같은 API
